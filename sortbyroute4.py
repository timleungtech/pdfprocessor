#sortdrivers for bread, dd, retail orders
import glob
import pdfplumber
import os
...
#bottom text start
        bluetransparent = Color( 0, 0, 255, alpha=0.25)
        packet = io.BytesIO()
        can = canvas.Canvas(packet, pagesize=letter)
        can.setFont("Courier-Bold", 42)
        can.setFillColor(bluetransparent)
        can.drawString(10, 15, f'ROUTE {route} - {customer}')
        can.setFont("Courier-Bold", 75)
        can.drawString(10, 300, f'{customer}')
        can.setFont("Courier-Bold", 42)
        can.drawString(10, 685, f'ROUTE {route} - {customer}')
        can.save()
